...
export const add = async (req, res) => {
    try {
        ...
        const finalPath = path.join(productDir, `image_1${fileExt}`);
        ...
        if (!fs.existsSync(productDir)) {
            ...
            const existingFiles = fs.readdirSync(productDir);
            const existingImage = existingFiles.find(file => file.startsWith("image_1"));
            if (existingImage) {
                const oldImagePath = path.join(productDir, existingImage);
        ...
        product.image_path = `/uploads/products/${productId}/image_1${fileExt}`;
        await product.save();
